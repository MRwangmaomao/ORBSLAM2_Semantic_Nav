...
{
    cv::Rect_<float> ir1(r1), ir2(r2);
    /* calculate center of rectangle #1  边框中心点 */
    cv::Point c1(ir1.x + ((int)ir1.width >> 1), ir1.y + ((int)ir1.height >> 1));// 边框 中心点1
    /* calculate center of rectangle #2  边框中心点 */
    cv::Point c2(ir2.x + ((int)ir2.width >> 1), ir2.y + ((int)ir2.height >> 1));// 边框 中心点2
    ...
    // 使用 平均尺寸  进行匹配度 加权 =====================================================
    double len_diag = sqrt(powf(((int)(ir1.width + ir2.width) >> 1), 2) + powf(((int)(ir1.height + ir2.height) >> 1), 2));
