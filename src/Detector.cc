...
Detector::Detector()
{
    ...
    det_net_ptr->load_param("/home/wpr/code/slam_sys/ORB_SLAM2_SSD_Semantic/perfect/Thirdparty/ncnn/model/mobilenetv2_ssdlite_voc.param");
    det_net_ptr->load_model("/home/wpr/code/slam_sys/ORB_SLAM2_SSD_Semantic/perfect/Thirdparty/ncnn/model/mobilenetv2_ssdlite_voc.bin");
...
// 格式化网络输入
    ...
    *net_in_ptr = ncnn::Mat::from_pixels_resize(bgr_img.data, ncnn::Mat::PIXEL_RGB, bgr_img.cols, bgr_img.rows, target_size, target_size);
    ...
    cv::Mat im = bgr_img.clone();
    // 将识别结果存入到object中
    for (int i=0; i<out.h; i++)
    {
        const float* values = out.row(i);// 每个检测结果
        std::cout << "-------------------find Object-------------------" << std::string(class_names[int(values[0])]) << std::endl;
        Object object;// 一个
        ...
        cv::Rect rec(object.rect.x, object.rect.y, object.rect.width, object.rect.height);
