...
// 1. 窗口设置 pangolin 胖果林 创建 地图 显示窗口=====1024×768=====
    pangolin::CreateWindowAndBind("MAP",1024,768);// 窗口名字和窗口大小=====
...
{
  pangolin::BindToContext("MAP");
}

void Viewer::SaveOctomap(const char *filename)
{
    mpMapDrawer->SaveOctoMap(filename);
}
